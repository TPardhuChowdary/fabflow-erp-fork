...
| [phase-10](./phase-10/) | Delivery Challans — extends the pre-existing `delivery_challans` table; dispatch-method fields, `items`/`project_entries`/`delivery_address` jsonb, new `customer_id` FK with `ON DELETE SET NULL` correcting a confirmed `deleteCustomer()` guard gap, `updated_at` maintenance. The first phase where independent verification caught a genuine defect (a self-contradictory `NOT NULL` + `ON DELETE SET NULL` combination on `customer_id`) after execution — disclosed, corrected, and re-verified live | ✅ Complete — all checks verified after one disclosed and corrected defect, 0 FAIL outstanding |
| phase-11+ | Production Stages, Material Requisitions (pending resolution of its unresolved three-way concept conflict), Machinery, Drawing Repository, QMS, Ledger & Reports (incl. Payables), Dashboard & Analytics, full integration | Not started |
...
- [phase10_architecture.md](./phase-10/phase10_architecture.md) — what was built and why, including the module-selection ranking once dependency count stopped differentiating candidates, and the disclosed `customer_id` NOT NULL / ON DELETE SET NULL conflict
- [phase10_security.md](./phase-10/phase10_security.md) — the security/integrity model, explaining why the corrected design matches the nullable-FK-target pattern already used for `vendor_id`/`inventory_item_id`
- [phase10_verification.md](./phase-10/phase10_verification.md) — verification methodology, with the full incident record of the one genuine defect this project's independent verification has caught after execution: found, disclosed, corrected, and re-verified live in the same session
- [phase10_completion_report.md](./phase-10/phase10_completion_report.md) — full results, including the one disclosed FAIL-then-fixed behavioral test
- [phase10_rollback.md](./phase-10/phase10_rollback.md) — rollback plan (documented, not executed), with a note distinguishing the archived corrected file from the briefly-live defective checksum
- [phase10_delivery_challans_FINAL.sql](./phase-10/phase10_delivery_challans_FINAL.sql) — the executed (corrected) migration itself, registered in `schema_migrations` as `20260806_010_phase10_delivery_challans`
